...
pub fn download<T: Terminal>(
    ...
    compare_prefix(&mut remote, &mut local, partial, |n| {
        ui.progress("校验已有下载内容", n, partial)
    })?;
    ...
    compare_prefix(&mut remote, &mut local, total, |n| {
        ui.progress("下载完成，正在逐字节校验", n, total)
    })?;
    ensure_source_unchanged(&before, &sftp.stat(Path::new(source))?)?;
    drop(local);
...
pub fn upload<T: Terminal>(
    ...
    let partial = match partial_stat.as_ref() {
        Some(stat) => {
            if !stat.is_file() {
                bail!("远程 .part 不是普通文件，拒绝写入")
            }
            let length = stat.size.context("无法读取 .part 文件大小")?;
            if length > total {
                bail!("远程 .part 比本地源文件更大，请改用新目标路径")
            }
            if !ui.confirm(
                "发现未完成的上传",
                &[
    ...
    compare_prefix(&mut local, &mut remote, partial, |n| {
        ui.progress("校验已有上传内容", n, partial)
    })?;
    local.seek(SeekFrom::Start(partial))?;
    remote.seek(SeekFrom::Start(partial))?;
    let mut copied = partial;
    ...
    local.seek(SeekFrom::Start(0))?;
    remote.seek(SeekFrom::Start(0))?;
    compare_prefix(&mut local, &mut remote, total, |n| {
        ui.progress("上传完成，正在逐字节校验", n, total)
    })?;
    let current = local.metadata()?;
    if current.len() != total
        || current.modified()? != modified
        || remote.stat()?.size != Some(total)
    {
