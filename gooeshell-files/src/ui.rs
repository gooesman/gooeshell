...
impl<T: Terminal> Ui<T> {
    ...
    pub fn draw(&mut self, title: &str, lines: &[(String, bool)], footer: &str) -> Result<()> {
        ...
        for (index, (line, selected)) in lines.iter().take(size.rows.saturating_sub(3)).enumerate()
        {
            changes.extend([
                Change::CursorPosition {
                    ...
                    y: Position::Absolute(index + 2),
                },
                Change::Attribute(AttributeChange::Background(if *selected {
                    AnsiColor::Navy.into()
                } else {
                    AnsiColor::Black.into()
                })),
                Change::Attribute(AttributeChange::Foreground(if *selected {
                    AnsiColor::White.into()
                } else {
                    AnsiColor::Silver.into()
                })),
                Change::ClearToEndOfLine(if *selected {
                    AnsiColor::Navy.into()
                } else {
                    AnsiColor::Black.into()
                }),
        ...
        if size.rows > 1 {
            changes.extend([
                Change::CursorPosition {
                    x: Position::Absolute(0),
                    y: Position::Absolute(size.rows - 1),
                },
    ...
    pub fn message(&mut self, title: &str, message: &str) -> Result<()> {
        ...
        let lines: Vec<_> = wrap_text(message, width)
            .into_iter()
            .map(|s| (s, false))
            .collect();
    ...
    pub fn prompt(&mut self, title: &str, initial: &str, secret: bool) -> Result<Option<String>> {
        ...
        loop {
            ...
            let display = if secret {
                ...
                format!("> {}_", tail_text(&value, width))
            };
            self.draw(
                title,
                &[(display, true)],
            ...
            match self.terminal.poll_input(None)? {
                ...
                Some(InputEvent::Key(KeyEvent {
                    key: KeyCode::Char('u'),
                    modifiers,
                })) if modifiers.contains(Modifiers::CTRL) => value.clear(),
                Some(InputEvent::Key(KeyEvent {
                    key: KeyCode::Char(c),
                    modifiers,
                })) if !c.is_control()
    ...
    pub fn progress(&mut self, phase: &str, done: u64, total: u64) -> Result<()> {
        if let Some(InputEvent::Key(KeyEvent { key, modifiers })) =
            self.terminal.poll_input(Some(Duration::ZERO))?
        {
            if key == KeyCode::Escape
                || (key == KeyCode::Char('c') && modifiers.contains(Modifiers::CTRL))
            {
