...
import LoginIdentitySelect, { useLoginIdentityLibrary, type IdentityChoice } from './LoginIdentitySelect';
...
export interface ConnectionDialogProps {
  ...
  onHostKeyPreference?: (preference: HostKeyPreference) => Promise<void>;
  onDelete?: () => void;
  onDuplicate?: () => void;
...
export default function ConnectionDialog({ profile, saved, groups, hostKeyPreferences, busy, error, onSave, onConnect, onCancelConnect, onHostKeyPreference, onDelete, onDuplicate, onPushKey, onManageIdentities, onClose }: ConnectionDialogProps) {
  ...
  const [fingerprintChoices, setFingerprintChoices] = useState<Record<string, boolean>>({});
  ...
  const hostPreferenceKey = JSON.stringify([normalizedHost(draft.host), draft.port]);
  const storedAutomaticHostKey = hostKeyPreferences.some(preference => preference.skipVerification && normalizedHost(preference.host) === normalizedHost(draft.host) && preference.port === draft.port);
  const automaticHostKey = fingerprintChoices[hostPreferenceKey] ?? storedAutomaticHostKey;
  ...
  const identityLibrary = useLoginIdentityLibrary();
  const identity = identityLibrary.identities.find(item => item.id === draft.loginIdentityId);
  const jumpIdentity = identityLibrary.identities.find(item => item.id === jump?.loginIdentityId);
  const selectIdentity = (value?: IdentityChoice) => {
    setPassword(''); setPassphrase(''); rememberTouched.current = false;
    setDraft(previous => ({ ...previous, loginIdentityId: value?.id, username: value?.username || identity?.username || previous.username, ...(value ? { auth: 'password' as const, privateKeyPath: undefined } : {}) }));
  };
  ...
  }, [draft.id, draft.host, draft.port, draft.username, draft.auth, draft.privateKeyPath, draft.loginIdentityId, identity?.version, identity?.username, identity?.hasPassword, identity?.remember, jump?.id, jump?.host, jump?.port, jump?.username, jump?.auth, jump?.privateKeyPath, jump?.loginIdentityId, jumpIdentity?.version, jumpIdentity?.username, jumpIdentity?.hasPassword, jumpIdentity?.remember]);

  const normalized = (): HostProfile => ({ ...draft, name: draft.name.trim() || draft.host.trim(), host: draft.host.trim().replace(/^\[|\]$/g, ''), username: (identity?.username || draft.username).trim(), privateKeyPath: draft.auth === 'key' ? draft.privateKeyPath?.trim() : undefined, groupId: draft.groupId || undefined,
    jumpHost: jump ? { ...jump, name: jump.name.trim() || jump.host.trim(), host: jump.host.trim().replace(/^\[|\]$/g, ''), username: (jumpIdentity?.username || jump.username).trim(), privateKeyPath: jump.auth === 'key' ? jump.privateKeyPath?.trim() : undefined } : undefined });
  ...
  const valid = () => {
    if ((draft.loginIdentityId && !identity) || (jump?.loginIdentityId && !jumpIdentity)) { setLocalError('请选择可用的登录身份，或改为手动填写。'); return false; }
    ...
    if (!draft.loginIdentityId && remember === 'persistent' && !status?.secureStorageAvailable) { setLocalError('系统加密存储暂不可用，请选择仅本次记住。'); return false; }
    if (jump) {
      ...
      if (!jump.loginIdentityId && jumpRemember === 'persistent' && !status?.secureStorageAvailable) { setLocalError('系统加密存储暂不可用，请调整跳板机密码保存方式。'); return false; }
    }
  ...
  const submit = async (connect: boolean) => {
    ...
    setLocalBusy(true);
    try {
      if (onHostKeyPreference && automaticHostKey !== storedAutomaticHostKey) await onHostKeyPreference({ host: normalizedHost(draft.host), port: draft.port, skipVerification: automaticHostKey });
      if (connect) await onConnect(normalized(), credentials(), favorite);
      else { await onSave(normalized(), favorite, credentials()); onClose(); }
  ...
  const openKeyPush = async () => {
    if (!onPushKey || blocked || statusLoading || !valid()) return;
    setLocalBusy(true);
    try {
      if (onHostKeyPreference && automaticHostKey !== storedAutomaticHostKey) await onHostKeyPreference({ host: normalizedHost(draft.host), port: draft.port, skipVerification: automaticHostKey });
      await onPushKey(normalized(), credentials(), favorite);
    } catch (reason) { setLocalError(errorMessage(reason)); }
    finally { setLocalBusy(false); }
  };
    ...
    <section className="modal connection-manager-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-label={profile ? '连接设置' : '新建 SSH 连接'}>
      ...
      <form id="connect-form" className="modal-body connection-form" onSubmit={event => { event.preventDefault(); void submit(true); }}>
        <fieldset className="connection-fields" disabled={blocked}>
          ...
          <div className="form-grid">
            <LoginIdentitySelect id="host-login-identity" value={draft.loginIdentityId} identities={identityLibrary.identities} disabled={identityLibrary.loading || blocked} onChange={selectIdentity} onManage={onManageIdentities} />
            <div className="form-field full"><label htmlFor="host-user">用户名</label><input id="host-user" value={identity?.username || draft.username} readOnly={!!draft.loginIdentityId} onChange={event => change('username', event.target.value)} autoComplete="off" /></div>

            {!draft.loginIdentityId && <div className="form-field full"><div className="radio-group" role="group" aria-label="身份验证方式">{([['password', '密码'], ['key', '私钥文件'], ['agent', 'SSH Agent']] as const).map(([id, label]) => <button key={id} type="button" aria-pressed={draft.auth === id} className={draft.auth === id ? 'active' : ''} onClick={() => change('auth', id)}>{label}</button>)}</div></div>}
            {draft.auth === 'password' && (!draft.loginIdentityId || !identity?.hasPassword) && <div className="form-field full"><label htmlFor="host-password">{draft.loginIdentityId ? '本次登录密码' : '登录密码'}</label><input id="host-password" type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="new-password" placeholder={!identityChanged && status?.hasPassword ? '已记住 · 留空保留原密码' : '输入登录密码'} /></div>}
            ...
            {!draft.loginIdentityId && <div className="form-field full"><label htmlFor="credential-remember">密码与口令保存方式</label><select id="credential-remember" disabled={statusLoading} value={remember} onChange={event => { rememberTouched.current = true; setRemember(event.target.value as CredentialUpdate['remember']); }}><option value="never">不记住</option><option value="session">仅本次使用期间记住</option><option value="persistent" disabled={!status?.secureStorageAvailable}>长期记住 · 系统加密存储{status && !status.secureStorageAvailable ? '（不可用）' : ''}</option></select><span className="hint">{statusLoading ? '正在读取密码保存状态…' : remember === 'persistent' ? '应用重启后仍可使用，可随时清除。' : remember === 'session' ? '关闭应用后清除，断线时仍可用于重连。' : '下次连接需要重新输入。'}</span></div>}
            {identityChanged && !draft.loginIdentityId && <p className="connection-inline-note full">连接地址、账号或认证方式已修改，请重新填写密码；原连接的密码不会自动用于新目标。</p>}
            {!draft.loginIdentityId && (hasStored || forgotten) && <div className="credential-status full"><ShieldCheck size={15} /><span>{forgotten && !hasStored ? '已清除记住的密码与口令' : '此连接已有记住的密码或口令'}</span>{hasStored && <button type="button" className="text-button" onClick={() => void forget()}>清除已记住的密码</button>}</div>}
          </div>
          <section className="connection-advanced">
            <button type="button" className="connection-advanced-toggle" aria-expanded={advancedOpen} aria-controls="connection-advanced-options" onClick={() => setAdvancedOpen(value => !value)}><span><strong>高级选项</strong><small>{jump ? `通过跳板机${jump.name || jump.host ? ` · ${jump.name || jump.host}` : ''}` : '收藏、分组、指纹、密钥与跳板机'}</small></span><ChevronDown size={16} /></button>
          ...
          {onHostKeyPreference && <label className="checkbox-row"><input type="checkbox" aria-label="自动接受服务器指纹" checked={automaticHostKey} onChange={event => setFingerprintChoices(previous => ({ ...previous, [hostPreferenceKey]: event.target.checked }))} />此地址自动接受服务器指纹</label>}
          {automaticHostKey && <p className="connection-inline-note">对 {draft.host}:{draft.port} 生效：不校验、不保存，也不弹出指纹确认。用于同一地址轮换设备的可信网络。</p>}
          <div className="form-field full">{!automaticHostKey && <label className="checkbox-row"><input type="checkbox" checked={draft.rememberHost} onChange={event => change('rememberHost', event.target.checked)} />允许保存服务器指纹（首次连接仍需确认）</label>}<span className="hint">{automaticHostKey ? '此地址已设为自动接受指纹：不校验、不保存，直接进行账号认证。' : '关闭后仅信任本次设备，不读取或保存旧指纹记录。'}</span></div>
          ...
          <p id="host-shell-integration-hint" className="connection-inline-note">下次连接使用 Linux Bash，读取 .bashrc，不读取登录 profile，也不永久修改服务器启动文件。标记位置可在“字体与外观”中设置；tmux、Vim 等全屏程序内暂不显示命令标记。</p>
          ...
          <div className="connection-section-label">跳板机</div>
              ...
              {jump && <div className="form-grid">
                ...
                <LoginIdentitySelect id="jump-login-identity" label="跳板机登录身份" value={jump.loginIdentityId} identities={identityLibrary.identities} disabled={identityLibrary.loading || blocked} onManage={onManageIdentities} onChange={value => { resetJumpSecrets(); change('jumpHost', { ...jump, id: crypto.randomUUID(), loginIdentityId: value?.id, username: value?.username || jumpIdentity?.username || jump.username, ...(value ? { auth: 'password', privateKeyPath: undefined } : {}) }); }} />
                <div className="form-field full"><label htmlFor="jump-user">跳板机用户名</label><input id="jump-user" value={jumpIdentity?.username || jump.username} readOnly={!!jump.loginIdentityId} onChange={event => changeJump('username', event.target.value)} autoComplete="off" /></div>
                {!jump.loginIdentityId && <div className="form-field full"><label htmlFor="jump-auth">跳板机身份验证</label><select id="jump-auth" value={jump.auth} onChange={event => changeJump('auth', event.target.value as JumpHostProfile['auth'])}><option value="password">密码</option><option value="key">私钥文件</option><option value="agent">SSH Agent</option></select></div>}
                {jump.auth === 'password' && (!jump.loginIdentityId || !jumpIdentity?.hasPassword) && <div className="form-field full"><label htmlFor="jump-password">{jump.loginIdentityId ? '跳板机本次登录密码' : '跳板机登录密码'}</label><input id="jump-password" type="password" value={jumpPassword} onChange={event => setJumpPassword(event.target.value)} autoComplete="new-password" placeholder={!statusLoading && status?.jump?.hasPassword ? '已记住 · 留空保留原密码' : '输入跳板机密码'} /></div>}
                ...
                {!jump.loginIdentityId && <div className="form-field full"><label htmlFor="jump-remember">跳板机密码与口令保存方式</label><select id="jump-remember" disabled={statusLoading} value={jumpRemember} onChange={event => { jumpRememberTouched.current = true; setJumpRemember(event.target.value as CredentialUpdate['remember']); }}><option value="never">不记住</option><option value="session">仅本次使用期间记住</option><option value="persistent" disabled={!status?.secureStorageAvailable}>长期记住 · 系统加密存储</option></select></div>}
                {jumpIdentityChanged && !jump.loginIdentityId && <p className="connection-inline-note full">跳板机地址、账号或认证方式已修改，请重新填写密码；不会更改其他连接中的跳板机设置。</p>}
                {!jump.loginIdentityId && (status?.jump?.hasPassword || status?.jump?.hasPassphrase) && !statusLoading && <div className="credential-status full"><ShieldCheck size={15} /><span>此跳板机已有记住的密码或口令</span><button type="button" className="text-button" onClick={() => void forgetJump()}>清除跳板机密码</button><span className="hint">复用此跳板机设置的连接也将需要重新输入密码。</span></div>}
              ...
              {profile && (onDuplicate || onDelete) && <><div className="connection-section-label">连接管理</div><div className="connection-management-actions">{onDuplicate && <button type="button" className="button secondary" onClick={onDuplicate}>新建同名终端</button>}{onDelete && <button type="button" className="text-button" onClick={onDelete}>删除连接…</button>}</div></>}
            </div>}
          </section>
          {identityLibrary.error && <p className="connection-inline-note">身份库暂不可用：{identityLibrary.error}</p>}
