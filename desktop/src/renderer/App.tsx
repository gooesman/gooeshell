...
export default function App() {
  ...
  useEffect(() => {
    ...
    const findAction = (combo: string) => { if (!combo) return undefined; const id = Object.entries(settings.shortcuts).find(([, value]) => value && value.toLowerCase() === combo.toLowerCase())?.[0]; return id && ['commands', 'connect', 'settings', 'sidebar', 'terminalHeader', 'previousTab', 'nextTab', 'files', 'maximize', 'fullscreen', 'zen'].includes(id) ? id : undefined; };
    const run = (id: string) => { if (id === 'commands') toggleCommands(); if ((id === 'previousTab' || id === 'nextTab') && tabs.length) { const index = tabs.indexOf(activeTabId); const next = tabs[(index + (id === 'previousTab' ? -1 : 1) + tabs.length) % tabs.length]; setActiveId(sessions.find(session => (session.tabId || session.id) === next)?.id || next); } if (id === 'connect') { setConnectionError(''); setConnection({ tabId: active ? undefined : activeId }); } if (id === 'settings') (setSettingsPage(undefined), setSettingsOpen(true)); if (id === 'sidebar') { if (zen) { setZen(false); setSidebarCollapsed(false); } else setSidebarCollapsed(value => !value); } if (id === 'terminalHeader') setTerminalHeaderVisible(value => !value); if (id === 'files') toggleFiles(); if (id === 'maximize') api.maximize(); if (id === 'fullscreen') void api.fullscreen().catch(e => notify(errorText(e), true)); if (id === 'zen') setZen(value => !value); };
    const withinTerminal = (target: EventTarget | null) => target instanceof Element && !!target.closest('.terminal-region .xterm');
    const keyboard = (event: KeyboardEvent) => {
      if (event.isComposing || event.keyCode === 229 || modalOpen || (event.target instanceof Element && event.target.closest('.terminal-paste-dialog'))) return;
      // The active multiline queue owns Enter and Ctrl+Enter even when assigned to an app action.
      if (event.key === 'Enter' && !event.shiftKey && !event.altKey && !event.metaKey && event.target instanceof Element && event.target.closest('.terminal-instance[data-active="true"]')?.querySelector('.terminal-paste-queue')) return;
      const id = findAction(keyCombo(event)); if (!id) return;
      if (!event.ctrlKey && !event.altKey && !event.metaKey && !/^F\d+$/.test(event.key) && !withinTerminal(event.target)) return;
      event.preventDefault(); event.stopImmediatePropagation(); if (!event.repeat) run(id);
    };
