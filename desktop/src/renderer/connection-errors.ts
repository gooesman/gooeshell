...
export function connectionErrorText(error: unknown): string {
  const text = unwrapError(error).replace(/^(?:(?:JUMP_)?AUTH_(?:REQUIRED|FAILED)|CREDENTIAL_[A-Z_]+|LOGIN_IDENTITY_[A-Z_]+|SSH_KEY_[A-Z_]+|KEY_PUSH_[A-Z_]+|CONNECTION_(?:CANCELLED|IDENTITY_CHANGED)|SUDO_PASSWORD_REQUIRED):\s*/, '');
