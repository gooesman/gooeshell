...
test('connection UI separates saving from connecting and preserves explicit credential choices', {
  ...
  for (const name of ['shellIntegrationOptInSavedWithoutConnecting', 'groupCollapseAndPhysicalSession', 'simplifiedConnectionSections', 'saveWithoutConnectingAndBlankSecretPreservation', 'statusRefreshPreservesUserChoice', 'newConnectionOptions', 'jumpPresetAndSeparateSecrets', 'jumpIdentityIsolationAndDisable', 'cancelPendingConnection', 'sharedIdentitiesSeparateTargetAndJump', 'manualSelectionDoesNotKeepIdentityReference', 'forgetCredentials']) assert.equal(result.checks[name], true, name);
