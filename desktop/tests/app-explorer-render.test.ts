...
test('App explorer manages files with explicit targets and independent terminal directory tracking', {
  skip: process.env.GOOESHELL_APP_EXPLORER_TEST !== '1' && 'Set GOOESHELL_APP_EXPLORER_TEST=1 with Electron installed', timeout: 150_000,
}, async t => {
  ...
  const timeout = setTimeout(() => child.kill(), 140_000); timeout.unref(); child.once('close', () => clearTimeout(timeout)); t.after(() => { if (child.exitCode === null) child.kill(); });
  ...
  for (const name of ['remoteColumnsSortWithArrows', 'paneSortingIsIndependentAndPersists', 'remoteContextCreatesFileAndFolder', 'existingFileIsPreserved', 'blankMenuCreatesFile', 'selectionDoesNotShiftRows', 'blankListClickClearsSelection', 'renameKeepsParent', 'deleteCancellationDoesNotMutate', 'multiDeleteIncludesFolderContents', 'localActionsMatchRemote', 'pendingSudoKeepsOriginalTarget', 'partialDeletionSudoSkipsCompleted', 'directoryTrackingIsPerTerminal', 'manualNavigationPausesFollowing', 'hiddenExplorerStopsPolling', 'closedSessionDisablesActions', 'transferChoiceCancellationDoesNotStart', 'ordinaryTransferRemainsAvailable', 'packedTransferKeepsOriginalTarget', 'uploadPickerKeepsOriginalTarget', 'draggedTransferOffersCompression', 'packedStagesCanCancelAndRetry', 'pendingTransferStopsWhenDisconnected', 'remoteDragKeepsOriginalSession', 'minimalFileControlsKeepFollow', 'transferSpeedAndEtaStayCurrent', 'transferTaskMenuStopsWork', 'deletingActiveTaskWaitsForStop', 'deletingFailedTaskOnlyRemovesQueueEntry', 'failedTaskCancellationKeepsRow']) assert.equal(result.checks[name], true, name);
  assert.deepEqual(result.errors, []);
  for (const name of ['verificationUsesIndependentProgress', 'preparingDoesNotShowCompletedTransferProgress', 'archiveVerificationAndStageChangesStayCurrent']) assert.equal(result.checks[name], true, name);
});
