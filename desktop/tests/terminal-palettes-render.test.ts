...
for (const domFallback of [false, true]) test(`actual TerminalView palette changes preserve terminal state (${domFallback ? 'forced DOM fallback' : 'default renderer'})`, {
  ...
  for (const name of ['exactInteriorSampling', 'legacyFollow', 'boldGlyphs', 'explicitColors', 'independentPalette', 'sameTerminal', 'statePreserved', 'backgroundImage', 'inputResponsive']) assert.equal(result.checks[name], true, name);
