...
const inspect = () => window.webContents.executeJavaScript(`(() => {
  ...
  return {text:lines.join('\\n'),renderedText:window.__fixtureLastRenderedText,cols:t.cols,rows:t.rows,type:b.type,renders:window.__fixtureRenders,canvas:document.querySelectorAll('canvas').length,synchronized:t.modes.synchronizedOutputMode,visibility:document.visibilityState};
})()`);
