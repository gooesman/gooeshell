...
async function run() {
  ...
  phase = 'maximize shortcut invokes window control without replacing terminals';
  const maximizeBefore = await workspaceSnapshot();
  await shortcut('F10'); await shortcut('F10');
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 2);
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'fullscreen').length`), 0);
  assert.deepEqual(await workspaceSnapshot(), maximizeBefore, 'maximizing and restoring cannot reconnect or replace terminal buffers');
  window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'F11' });
  window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'F11' }); await delay(35);
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'fullscreen').length`), 1);
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 2);
  result.checks.maximizeShortcutPreservesWorkspace = true;
  phase = 'settings draft survives the refresh shortcut';
  await click('设置'); await click('字体与外观');
  await shortcut('F10');
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 2, 'settings keeps key capture instead of maximizing');
  ...
  phase = 'multiline paste owns Ctrl+Enter over a customized App shortcut';
  await click('设置'); await click('键盘与鼠标'); await click('录入最大化 / 还原窗口快捷方式');
  window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Enter', modifiers: ['control'] });
  window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Enter', modifiers: ['control'] }); await delay(35);
  await click('保存设置'); await noDialog();
  const controlEnter = async () => {
    window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Enter', modifiers: ['control'] });
    window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Enter', modifiers: ['control'] }); await delay(35);
  };
  await evaluate(`document.querySelector('[data-terminal-session][data-active="true"] .xterm-helper-textarea').focus()`);
  await controlEnter();
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 3, 'customized shortcut works outside a paste queue');
  await evaluate(`window.appConnectionsFixture.clipboardText = 'echo first\\necho second\\necho third'`);
  await shortcut('V'); await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-dialog'))`), 'multiline chooser');
  await controlEnter(); await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-queue[data-paste-line="1"]'))`), 'keyboard chooses line mode');
  await controlEnter();
  assert.equal(await evaluate(`document.querySelector('.terminal-paste-queue').dataset.pasteLine`), '1', 'next line waits for current Enter');
  window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Enter' });
  window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Enter' }); await delay(35);
  await controlEnter(); await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-queue[data-paste-line="2"]'))`), 'Ctrl+Enter fills the next line');
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 3, 'paste chooser and queue must not invoke the customized App action');
  await click('取消剩余'); await until(() => evaluate(`!document.querySelector('.terminal-paste-queue')`), 'paste cancelled');
  await controlEnter();
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 4, 'custom shortcut resumes after cancellation');
  await click('设置'); await click('键盘与鼠标'); await click('录入最大化 / 还原窗口快捷方式');
  const plainEnter = async () => {
    window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Enter' });
    window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Enter' }); await delay(35);
  };
  await plainEnter(); await click('保存设置'); await noDialog();
  await evaluate(`document.querySelector('[data-terminal-session][data-active="true"] .xterm-helper-textarea').focus()`);
  await plainEnter();
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 5, 'custom Enter works outside the paste queue');
  await shortcut('V'); await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-dialog'))`), 'second multiline chooser');
  await controlEnter(); await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-queue[data-paste-line="1"]'))`), 'second line queue');
  await plainEnter(); await controlEnter();
  await until(() => evaluate(`Boolean(document.querySelector('.terminal-paste-queue[data-paste-line="2"]'))`), 'ordinary Enter submits despite custom App mapping');
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 5);
  await click('取消剩余'); await until(() => evaluate(`!document.querySelector('.terminal-paste-queue')`), 'second queue cancelled');
  await plainEnter();
  assert.equal(await evaluate(`window.appConnectionsFixture.calls.filter(call => call.method === 'maximize').length`), 6, 'custom Enter resumes after cancellation');
  await click('设置'); await click('键盘与鼠标'); await click('录入最大化 / 还原窗口快捷方式');
  await shortcut('F10'); await click('保存设置'); await noDialog();
  result.checks.pasteQueueOverridesCustomAppShortcut = true;
