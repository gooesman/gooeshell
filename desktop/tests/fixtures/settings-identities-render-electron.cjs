...
async function run() {
  ...
  assert.equal(await evaluate(`document.querySelector('[aria-label="录入最大化 / 还原窗口快捷方式"]').textContent`), 'Ctrl + Shift + F10');
  await click('录入最大化 / 还原窗口快捷方式'); await keyboard('F11'); assert.match(await evaluate('document.querySelector(".form-error").textContent'), /无边框全屏/);
  await keyboard('F10', ['control', 'alt']);
  assert.equal(await evaluate(`document.querySelector('[aria-label="录入最大化 / 还原窗口快捷方式"]').textContent`), 'Ctrl + Alt + F10');
  ...
  assert.equal(await evaluate('window.settingsIdentityFixture.savedSettings.shortcuts.maximize'), 'Ctrl+Alt+F10');
  assert.equal(await evaluate('window.settingsIdentityFixture.savedSettings.shortcuts.fullscreen'), 'F11');
  result.checks.maximizeShortcutConfiguresAndPersists = true;
