...
async function run() {
  ...
  assert.equal(await evaluate(`Boolean(document.getElementById('host-shell-integration'))`), false);
  await advanced();
  assert.equal(await evaluate(`document.getElementById('host-shell-integration').checked`), false);
  assert.match(await evaluate(`document.getElementById('host-shell-integration-hint').textContent`), /不永久修改服务器启动文件/);
  await checkbox('命令识别（Bash）', true);
  await checkbox('命令识别（Bash）', false);
  assert.equal(await evaluate(`document.getElementById('host-shell-integration').checked`), false);
  await checkbox('命令识别（Bash）', true);
  ...
  assert.equal(saved.profile.shellIntegration, true); result.checks.shellIntegrationOptInSavedWithoutConnecting = true;
