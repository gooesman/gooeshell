...
test('real Electron IPC saves connection credentials, reconnects, sends sudo privately and cancels pending authentication',{
  skip:process.env.GOOESHELL_CONNECTION_MAIN_TEST==='1'?false:'Set GOOESHELL_CONNECTION_MAIN_TEST=1 after building the desktop app',timeout:60000,
},async t=>{
  ...
  for(const check of ['emptyDraftStatus','saveWithoutConnecting','directConnect','sudoPrivateInput','reconnect','failedAuthPreservesSecret','cancelHandshake','renameDuringConnect','identityInvalidates','encryptedDisk','forgetEvenLocked','forgetDuringAuth','identityRoundTripDuringAuth','deleteDuringAuth','workerRecovery','editorGuardIntact'])assert.equal(result.checks[check],true,'Missing check: '+check);
});
