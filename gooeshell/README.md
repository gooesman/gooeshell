...
首版默认使用已在当前 Windows 设备验证的 OpenGL 渲染。快捷键按键位识别，避免大小写锁定或输入法改变操作组合。
...
主机列表只保存用户名、主机、端口与信任偏好。永久主机指纹保存在 `data/known_hosts`。选择“仅本次信任”时，指纹写入本次启动专用的临时文件，仍会检查主机身份；应用正常退出时启动器删除这个文件。系统崩溃或强制终止启动器时临时文件可能保留在 `data/` 或 `data/sessions/` 中。终端连接与文件面板共用选定的指纹文件，但使用独立 SSH 连接。
