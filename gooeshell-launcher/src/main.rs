...
fn launch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = executable.parent().ok_or("Cannot locate application directory")?;
    let config = root.join("gooeshell").join("gooeshell.lua");
    let gui = root.join(if cfg!(windows) { "wezterm-gui.exe" } else { "wezterm-gui" });
    ...
    cmd.arg("--config-file").arg(config)
        .arg("start").arg("--always-new-process")
    ...
    if let Some(home) = std::env::var_os("USERPROFILE").or_else(|| std::env::var_os("HOME")) {
        if Path::new(&home).is_dir() { cmd.current_dir(home); }
    }
    ...
    let status = result?;
    if !status.success() {
        return Err(format!("终端启动失败（{status}）。请查看 gooeshell 日志或重新解压程序。").into());
    }
    Ok(())
}
...
#[cfg(not(windows))]
fn display_error(message: &str) { eprintln!("gooeshell: {message}"); }
